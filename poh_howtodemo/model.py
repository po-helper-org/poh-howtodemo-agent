...
HTTP = "http"
CLI = "cli"
BROWSER = "browser"      # ждёт замера памяти на стенде
# Утверждение о результате УЖЕ исполненного шага, а не новое действие: «вижу
# код 200», «в теле есть uptime_sec». Сценарии людей цепочные сплошь и рядом —
# один запрос и три проверки о нём.
ASSERT = "assert"
...
class Action:
    ...
    body: dict | None = None
    command: str = ""
    # Номер шага, о котором утверждение. Ноль — привяжет код, взяв последний
    # исполненный. Модель сюда не пишет: номер попадает в отчёт, а всё, что
    # попадает в вердикт и ссылки, считает код.
    on: int = 0
...
class StepResult:
    ...
    source: str = ""
    # Для утверждения — номер шага, по ответу которого оно проверено.
    on_step: int = 0
